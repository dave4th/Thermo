...
# Parametri generali
TestoPagina="Configurazione server MQTT (Level1)"
DirBase="/var/www"
ConfigFile=DirBase+"/conf/thermo.json"
WriteFile="/cgi-bin/writemqttserver.py"
# Redis "key"
RedisKey = "mqttbroker:server:message"
...
if not MyDB.exists(RedisKey):
    MyDB.hmset(RedisKey,{"hostname":"nessuno","port":1883,"user":"","password":""})
...
for i in MyDB.hkeys(RedisKey):
    ...
    if flt.Decode(i) == "hostname":
        ...
        print (mhl.MyTextForm(flt.Decode(i),flt.Decode(MyDB.hget(RedisKey,i)),"20","",""))
    else:
